...
//    LOGI(@"Carrier Name: %@ mcc: %@ mnc: %@", carrierName, mcc, mnc);
    return carrierName;
}
...
+ (NSArray *)getDNSByHostname:(NSString *)hostname{
    ...
    hostRef = CFHostCreateWithName(kCFAllocatorDefault, (CFStringRef)CFBridgingRetain(hostname));
    if (hostRef) {
