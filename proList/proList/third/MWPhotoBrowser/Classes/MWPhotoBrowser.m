...
- (void)viewDidLoad {
    ...
    if (self.disPlayEditButton) {
        _editButton = [[UIBarButtonItem alloc] initWithTitle:@"编辑" style:UIBarButtonItemStylePlain target:self action:@selector(editButtonPressed:)];
        
        _reloadPhotoButton = [[UIBarButtonItem alloc] initWithTitle:@"重拍" style:UIBarButtonItemStylePlain target:self action:@selector(reloadPhotoButtonPressed:)];
        
        _cancelButton = [[UIBarButtonItem alloc] initWithTitle:@"取消" style:UIBarButtonItemStylePlain target:self action:@selector(cancelButtonPressed:)];
        
    }else if (self.displayActionButton) {
        _actionButton = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemAction target:self action:@selector(actionButtonPressed:)];
        if (self.disPlayRotateImage) {
            _rotateButton =  [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"zj_answer_image_rate"] style:UIBarButtonItemStylePlain target:self action:@selector(rotateButtonClick:)];

        }
    }
    //zj_answer_image_rate@2x
    ...
    if (self.isShowPageContol) {
        self.pageControl = [[UIPageControl alloc] initWithFrame:CGRectMake(0,    self.view.height - 44 - 20 - iPhoneX_BOTTOM_HEIGHT, SCREEN_WIDTH, 20)];
        self.pageControl.pageIndicatorTintColor = UIColorFromRGB(0x666666);
        self.pageControl.currentPageIndicatorTintColor = UIColorFromRGB(0xcccccc);
...
- (void)rotateButtonClick:(UIButton *)sender {
    ...
    if (_doneButton) {
        ...
        if ([_delegate respondsToSelector:@selector(photoBrowserDidFinishModalPresentation:)]) {
            ...
            UIView *masView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
            masView.backgroundColor = [UIColor blackColor];
            [[UIApplication sharedApplication].keyWindow addSubview:masView];
            ...
            // 给你的播放视频的view视图设置旋转
            if (self.isShowLandSpace) {
                imageView.transform = CGAffineTransformIdentity;//CGAffineTransformMakeRotation(-M_PI_2);
            }else {
                imageView.transform = CGAffineTransformIdentity;//CGAffineTransformMakeRotation(M_PI_2);
            }
            // 开始旋转
            ...
            [self dismissViewControllerAnimated:NO completion:^{
                [[NSNotificationCenter defaultCenter] postNotificationName:KBROWSERNEEDRORATE object:nil];
            }];
...
- (void)performLayout {
    ...
    // Right - Action
    if (_editButton && _reloadPhotoButton) {
        [items removeAllObjects];
        fixedSpace.width = SCREEN_WIDTH - 120;
        [items addObject:_editButton];
        [items addObject:fixedSpace];
        [items addObject:_reloadPhotoButton];
    }else if (_actionButton && !(!hasItems && !self.navigationItem.rightBarButtonItem)) {
        [items addObject:_actionButton];
        if (_rotateButton) {// 图片旋转
              [items removeAllObjects];
              fixedSpace.width = SCREEN_WIDTH - 90;
	...
	for (NSUInteger index = (NSUInteger)iFirstIndex; index <= (NSUInteger)iLastIndex; index++) {
		if (![self isDisplayingPageForIndex:index]) {
            ...
            if (self.displaySelectionButtons) {
                ...
                [selectedButton setImage:[UIImage imageNamed:@"icon-image-deselected"] forState:UIControlStateNormal];
                ...
                if (self.customImageSelectedIconName) {
                    ...
                    selectedOnImage = [UIImage imageNamed:@"icon-image-selected"];
                }
                [selectedButton setImage:selectedOnImage forState:UIControlStateSelected];
//                [selectedButton sizeToFit];
                [selectedButton setFrame:CGRectMake(0, 0, 30, 30)];
...
- (CGRect)frameForToolbarAtOrientation:(UIInterfaceOrientation)orientation {
	...
	return CGRectIntegral(CGRectMake(0, self.view.bounds.size.height - height - kTabbarSafeBottomMargin, self.view.bounds.size.width, height));
}
...
- (CGRect)frameForSelectedButton:(UIButton *)selectedButton atIndex:(NSUInteger)index {
    ...
    CGRect selectedButtonFrame = CGRectMake(pageFrame.origin.x + pageFrame.size.width - selectedButton.frame.size.width,
                                            padding + yOffset,
...
- (void)updateNavigation {
    ...
    if ([photo underlyingImage] == nil || ([photo respondsToSelector:@selector(isVideo)] && photo.isVideo)) {
        ...
        _reloadPhotoButton.enabled = NO;
        _reloadPhotoButton.tintColor = [UIColor clearColor];
        _editButton.tintColor = [UIColor clearColor];
        _actionButton.tintColor = [UIColor clearColor]; // Tint to hide button
    } else {
        _editButton.enabled = YES;
        _reloadPhotoButton.enabled = YES;
        _actionButton.enabled = YES;
        _actionButton.tintColor = nil;
        _reloadPhotoButton.tintColor = nil;
...
- (void)selectedButtonTapped:(id)sender {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(photoBrowser:canSelectedAtIndex:selectedChanged:)]) {
        if (![self.delegate photoBrowser:self canSelectedAtIndex:index selectedChanged:selectedButton.selected]) {
            return;
        }
    }
...
- (void)_playVideo:(NSURL *)videoURL atPhotoIndex:(NSUInteger)index {
    ...
    [AppNavigator showModalViewController:_currentVideoPlayerViewController animated:YES];
...
- (void)setCurrentPhotoIndex:(NSUInteger)index {
    ...
    _currentPageIndex = index;
    _pageIndexBeforeRotation = index;
	if ([self isViewLoaded]) {
...
- (void)doneButtonPressed:(id)sender {
    ...
    if (_editImage) {
        if (_delegate && [_delegate respondsToSelector:@selector(photoBrowser:editImage:)]) {
            [_delegate photoBrowser:self editImage:_editImage];
        }
    }
    ...
    if (_doneButton) {
        ...
        if ([_delegate respondsToSelector:@selector(photoBrowserDidFinishModalPresentation:)]) {
            ...
            [self dismissViewControllerAnimated:YES completion:^{              
                [[NSNotificationCenter defaultCenter] postNotificationName:KCOMFIRMCHANGEIMAGE object:nil];
            }];
        }
    }
}
    ...
    if (_doneButton) {
        ...
        if ([_delegate respondsToSelector:@selector(photoBrowserDidFinishModalPresentation:)]) {
            ...
            if ([self numberOfPhotos] > 0 && [photo underlyingImage]) {
                UIImage *editImage = [photo underlyingImage];
                if (_delegate && [_delegate respondsToSelector:@selector(photoBrowser:needEditImage:)]) {
                    [_delegate photoBrowser:self needEditImage:editImage];
                }
                _delegate = nil;
                [self dismissViewControllerAnimated:NO completion:^{
                     [[NSNotificationCenter defaultCenter] postNotificationName:kEYSHOWEDITBROWER object:nil];
                }];
                
                
//                WeakSelf
//                CustomDefineCameraViewController *customVC  = [[CustomDefineCameraViewController alloc] initWithImage:editImage];
//                customVC.photoBlock = ^(UIImage *image) {
//                    _editImage = image;
//                     MWPhoto *newPhoto = [[MWPhoto alloc] initWithImage:image];
//                    [_photos replaceObjectAtIndex:_currentPageIndex withObject:newPhoto];
//                    [weakSelf reloadData];
//                };
//                [AppNavigator showModalViewController:customVC animated:YES];
            }
        }
    }
...
- (void)reloadPhotoButtonPressed:(id)sender {
    if (_doneButton) {
        if (self.enableGrid) {
            if (self.startOnGrid && !_gridController) {
                [self showGrid:YES];
        ...
        if ([_delegate respondsToSelector:@selector(photoBrowserDidFinishModalPresentation:)]) {
            ...
            _delegate = nil;
            [self dismissViewControllerAnimated:NO completion:^{
                [[NSNotificationCenter defaultCenter] postNotificationName:KWRONGBOOKRELOADTAKEIMAGE object:nil];
